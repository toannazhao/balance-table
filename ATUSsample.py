...
def atusSampleAnalysis(sex, inputFile, outputFile):
    ...
    df['CHILD'], df['MAR'], df['ACTIVITY_LONG'] = np.nan, np.nan, np.nan #create new empty columns

    #filter for specified sex: 1 (male), 2 (female)
    df = df.loc[df['SEX'] == int(sex)]
